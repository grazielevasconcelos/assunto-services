...
namespace :server do
  ...
  task :start do
    ...
    system 'bundle exec puma -d --control-url tcp://127.0.0.1:9293 --control-token foo config.ru --redirect-stdout test-server-stdout.log --redirect-stderr test-server-stderr.log'
  end
